...
export async function loadAssertionRows(propertyId: string): Promise<AssertionRow[]> {
  const sql = getSql();
  return sql<AssertionRow[]>`
    SELECT
      a.assertion_id, a.property_id, a.field_key, a.value_json, a.source_id, a.source_type,
      a.effective_at, a.observed_at, a.confidence, a.status, a.created_at,
...
export async function insertAssertion(input: {
  ...
  await sql`
    INSERT INTO assertions (
      ...
      ${new Date()}, ${input.confidence ?? 1}, ${input.status ?? "accepted"}
    )
  `;
  await emitEvent({
